...
function blocoNota(){
  ...
  h+='<div style="border:1px solid var(--line);border-radius:var(--r);padding:12px;display:grid;gap:10px">'
   +'<div class="scan"><label class="f" style="flex:1">Item<input id="n_it" list="dlItens" placeholder="código ou descrição" autocomplete="off"></label>'
   +'<button class="btn" id="n_scan" type="button" title="Ler código de barras">Ler código</button></div>'
   +'<datalist id="dlItens">'+S.itens.filter(i=>i.ativo).map(i=>'<option value="'+esc(i.codigo)+'">'+esc(i.descricao)+' · saldo '+NUM(i.saldo,2)+' '+esc(i.unidade)+'</option>').join("")+'</datalist>'
   +'<div class="grid g3">'
  ...
  if(L.length){
    h+='<div class="tw"><table><thead><tr><th>Item</th><th class="num">Total</th><th></th></tr></thead><tbody>'
     +L.map((l,ix)=>'<tr><td><div class="cod">'+esc(l.codigo)+'</div><div>'+esc(l.descricao)+'</div>'
...
function addLinhaNota(){
  ...
  if(!it.ativo){toast("O item "+it.codigo+" está inativo.","e");return;}
...
function dlgPecasFrota(codigo){
  ...
  if(!movs.length){
    ...
    corpo='<div class="note" style="margin-bottom:12px"><b>'+INT(itens.length)+'</b> peça(s) diferente(s) · <b>'+INT(movs.length)+'</b> lançamento(s) · total <b>'+BRL(total)+'</b>'+(periodo?' · '+esc(periodo):'')+'</div>'
     ...
     +itens.map(i=>'<tr><td><div class="cod">'+esc(i.codigo)+'</div><div>'+esc(i.descricao)+'</div>'
     ...
     +movs.map(m=>{
       ...
       return '<tr><td><div class="cod">'+esc(m.codigo)+'</div><div>'+esc(m.descricao)+'</div>'
