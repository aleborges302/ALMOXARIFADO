...
const S={user:null,perfil:null,perfis:[],cfg:null,itens:[],movs:[],frota:[],forn:[],ferr:[],emps:[],invs:[],
...
function vAjustes(el){
  ...
  h+='<div class="panel"><header><h3>Usuários</h3><div class="r">'+INT(S.perfis.length)+'</div></header>'
   +'<div class="tw"><table><thead><tr><th>Nome</th><th>Perfil</th><th>Situação</th></tr></thead><tbody>'
   +S.perfis.map(u=>'<tr><td>'+esc(u.nome)+(u.id===(S.user&&S.user.id)?' <span class="tag t-pri">você</span>':'')+'</td>'
     ...
     +'<td>'+(ehAdmin()?'<select data-au="'+u.id+'"><option value="1"'+(u.ativo?" selected":"")+'>Ativo</option><option value="0"'+(!u.ativo?" selected":"")+'>Bloqueado</option></select>':(u.ativo?'<span class="tag t-ok">ativo</span>':'<span class="tag t-crit">bloqueado</span>'))+'</td></tr>').join("")
   +'</tbody></table></div>'
   +'<div class="pad fine">Novos usuários são criados no painel do Supabase em <b>Authentication › Users</b>. Assim que a conta existe, a pessoa aparece aqui e você define o perfil.</div></div>';
